  ...
  <div class="share">
    ...
    <el-card style="margin-top:10px;">
      ...
      <div class="contentBox">
        <div
          class="ttbox"
          v-for="(item,index) in myShareList"
          :key="item.id"
          @click="toShareList(item.LSBH)"
        >
          <el-card class="contentCard">
            <div slot="header">
              <span class="bood">{{item.ZC_HTMC}}</span>
            </div>
            <p>合同金额：{{item.ZC_HTJE}}万元</p>
            <p>发票金额：{{item.ZC_FPJE}}万元</p>
            <p>供应商信息：{{item.ZC_GYSXX}}</p>
            <p>使用人：{{item.ZC_SYR}}</p>
            <div class="imgBox">
